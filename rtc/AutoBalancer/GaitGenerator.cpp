...
#define deg2rad(deg) (deg * M_PI / 180)
  ...
  void refzmp_generator::calc_current_refzmp (hrp::Vector3& ret, std::vector<hrp::Vector3>& swing_foot_zmp_offsets, const double default_double_support_ratio_before, const double default_double_support_ratio_after, const double default_double_support_static_ratio_before, const double default_double_support_static_ratio_after)
  {
    ...
    size_t double_support_count_half_before = default_double_support_ratio_before * one_step_count + 1;
    size_t double_support_count_half_after = default_double_support_ratio_after * one_step_count + 1;
                                                  ...
                                                  const double delay)
  {
    ...
    foot_guided_controller_ptr = new foot_guided_controller<3>(dt, cur_refcog(2) - rg.get_refzmp_cur()(2), cur_refcog, total_mass, fg_zmp_cutoff_freq, total_external_force_z, gravitational_acceleration);
    ...
    // double omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / (cur_cog - rg.get_refzmp_cur())(2));
                                  ...
                                  const double t_squat, const double t_flight)
  {
    ...
    hrp::Matrix33 mid_rot;
    calc_foot_origin_rot(mid_rot, initial_jump_midcoords.rot, ez);
    hrp::Vector3 tmp_trans = mid_rot * trans;
    jump_last_cp = cur_cog + tmp_trans + jump_midcoords.rot * zmp_off;
    ...
    foot_guided_controller_ptr = new foot_guided_controller<3>(dt, cur_refcog(2) - start_ref_coords.pos(2), cur_refcog, total_mass, fg_zmp_cutoff_freq, total_external_force_z, gravitational_acceleration);
    foot_guided_controller_ptr->set_dc_off(hrp::Vector3::Zero());
    if (!double_support_zmp_interpolator->isEmpty()) double_support_zmp_interpolator->clear();
  }
    ...
    if (is_preview) {
      ...
      if (lcg.get_lcg_count() == static_cast<size_t>(footstep_nodes_list[lcg.get_footstep_index()][0].step_time/dt * 1.0) - 1 && lcg.get_footstep_index() > 0) {
        ...
        if (lr_region[cur_leg == RLEG ? LLEG : RLEG]) {
          ...
          hrp::Vector3 prev_fs_pos_relative = preprev_fs_rot.transpose() * (prev_fs_pos - preprev_fs_pos); // preprev foot frame
          ...
          if (prev_fs_pos_relative(2) > stair_thre) {
            if (prev_fs_pos_relative(0) > 0.0) overwritable_stride_limitation[0] = up_max_stride;
            else overwritable_stride_limitation[3] = up_max_stride;
          } else if (prev_fs_pos_relative(2) < - stair_thre) {
            if (prev_fs_pos_relative(0) > 0.0) overwritable_stride_limitation[0] = down_max_stride;
      ...
      double cur_omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / foot_guided_controller_ptr->get_dz());
      // double ref_omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / (cur_refcog - rg.get_refzmp_cur())(2));
      double ref_omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / foot_guided_controller_ptr->get_dz());
  ...
  bool gait_generator::proc_one_tick_jump (hrp::Vector3 cur_cog, const hrp::Vector3& cur_cogvel, const hrp::Vector3& target_cog)
  {
    ...
    foot_guided_controller_ptr->set_dz(dz(2), total_external_force_z);
    // hrp::Vector3 dz = hrp::Vector3(0, 0, foot_guided_controller_ptr->get_dz());
    double cur_omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / dz(2));
    double ref_omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / dz(2));
    act_cp = cur_cog + cur_cogvel / cur_omega + dc_off;
    ref_cp = cur_refcog + cur_refcogvel / ref_omega;
  ...
  void gait_generator::update_foot_guided_controller (bool& solved, const hrp::Vector3& cur_cog, const hrp::Vector3& cur_cogvel, const hrp::Vector3& cur_refcog, const hrp::Vector3& cur_refcogvel, const hrp::Vector3& target_cog)
  {
    ...
    if (lcg.get_lcg_count() >= static_cast<size_t>(fg_step_count * (1 - default_double_support_ratio_before))) {
      ...
      if (step_num_phase != LAST && step_num_phase != FIRST) remain_count = traj_remain_count;
    } else if (lcg.get_lcg_count() <= static_cast<size_t>(fg_step_count * default_double_support_ratio_after)) {
      walking_phase = DOUBLE_AFTER;
    } else {
      traj_remain_count -= static_cast<size_t>(fg_step_count * default_double_support_ratio_after) + 1;
    }
    // remain_count does not become 0
    remain_count++;
    traj_remain_count++;
    ...
    if (!refzmp_exist_p) {
      ...
      prev_que_rzmp = cur_ref_zmp;
    }

    // TODO: num_preview_step > 2もサポートする
    ...
    if (step_index > 0 && walking_phase == DOUBLE_BEFORE) {
        next_dist_steps = dist_steps;
    ...
    if (use_act_states &&
        num_preview_step == 1 && step_num_phase == LAST &&
        walking_phase != DOUBLE_BEFORE) {
      remain_count = std::max(static_cast<size_t>(remain_count), static_cast<size_t>(footguided_balance_time_const/dt));
      traj_remain_count = std::max(static_cast<size_t>(remain_count), static_cast<size_t>(footguided_balance_time_const/dt));
    }
    ...
    if (num_preview_step >= 1) {
      ...
      if (walking_phase == DOUBLE_BEFORE &&
        ...
        if (step_num_phase != BEFORE_LAST && step_num_phase != LAST) ref_zmp_traj.push_back(LinearTrajectory<hrp::Vector3>(mid_pos, next_pos, (footstep_nodes_list[step_index + (step_num_phase == LAST ? 0 : 1)].front().step_time * default_double_support_ratio_before) + dt));
      } else {
    ...
    if (num_preview_step >= 2) {
      last_cp = next2_pos;
      bool not_last_second = (!(step_num_phase == BEFORE2_LAST && walking_phase != DOUBLE_BEFORE) &&
                              !(step_num_phase == BEFORE_LAST && walking_phase == DOUBLE_BEFORE));
      if (not_last_second) calc_last_cp(last_cp, cur_step);
      ...
      if (not_last_second) ref_zmp_traj.push_back(LinearTrajectory<hrp::Vector3>(mid2_pos, next2_pos, (footstep_nodes_list[step_index + (step_num_phase == LAST ? 0 : (step_num_phase == BEFORE_LAST ? 1 : 2))].front().step_time * default_double_support_ratio_before)));
    }
    ...
    if (num_preview_step == 1 &&
        (walking_phase == DOUBLE_BEFORE || walking_phase == DOUBLE_AFTER)) {
      double tmp_ratio = 0.0;
      if (double_support_zmp_interpolator->isEmpty()) {
        double tmp_time = remain_count * dt;
      ...
      hrp::Vector3 tmp_zmp = (use_act_states && is_interpolate_zmp_in_double) ? cur_ref_zmp : feedforward_zmp;
  ...
  {
    double omega = std::sqrt((gravitational_acceleration - total_external_force_z/total_mass) / foot_guided_controller_ptr->get_dz());
    ...
    if (lcg.get_lcg_count() >= static_cast<size_t>(footstep_nodes_list[lcg.get_footstep_index()][0].step_time/dt * (default_double_support_ratio_after + margin_time_ratio)) &&
        lcg.get_lcg_count() < static_cast<size_t>(footstep_nodes_list[lcg.get_footstep_index()][0].step_time/dt * (1.0 - default_double_support_ratio_before)) - 1 &&
        // !(lcg.get_lcg_count() <= static_cast<size_t>(footstep_nodes_list[lcg.get_footstep_index()][0].step_time/dt * 0.5) - 1 && act_contact_states[0] && act_contact_states[1])
        !(act_contact_states[0] && act_contact_states[1])
        ) {
    ...
    use_roll_flywheel = use_pitch_flywheel = false;
    {
      ...
      flywheel_tau = total_mass * (gravitational_acceleration - total_external_force_z/total_mass) * hrp::Vector3(-short_of_zmp(1), short_of_zmp(0), 0);
